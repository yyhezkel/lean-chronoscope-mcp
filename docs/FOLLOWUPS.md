...
Known deferrals and rough edges, not blocking v1.1.0. Ordered roughly by value.
...
- **Sessions accumulate; nothing closes them mid-run.** The smoke scripts create a new `--session` each run and never call `session_close`, so the daemon piled up 37 BrowserContexts / 121 session dirs in one session of testing. Retention sweep only runs at daemon start and only by age (7 days). Options: smokes should `session_close` when done; add an idle-session reaper (close contexts with no activity for N minutes); cap concurrent contexts. Workaround today: restart the container to drop stale contexts.

## MCP client integration note

- **This runtime only mounts HTTP MCP servers into the agent session — not stdio.** Confirmed empirically: every HTTP server (playwright, Canva, Gmail, Drive) loaded; the only stdio server (browser-mcp via `docker exec -i … node`) never did, despite `claude mcp list` reporting it "✓ Connected" (the CLI spawns the stdio process locally just to health-check). No number of session restarts fixes a stdio registration here.
  - **Resolution:** register browser-mcp over its HTTP+SSE bridge (M4.5). `docker/docker-compose.yml` publishes `127.0.0.1:8780` and sets `BROWSER_MCP_HTTP_TOKEN`; register with `claude mcp add browser-mcp -s user --transport http http://127.0.0.1:8780/mcp --header "Authorization: Bearer <token>"`. See `CLAUDE.md` → MCP client integration.
...
- **Claude Code defers MCP tool *schemas***, however (confirmed empirically): at mount it injects only tool *names* into the model's context and fetches each full JSON schema on demand via its `ToolSearch` mechanism. So browser-mcp's `tools/list` payload (~5,258 tok in full mode) is the *server-emitted* size, **not** what sits in the model's context at mount — actual mounted cost is closer to the names list. This makes server-side progressive disclosure (e.g. `--gateway` mode) **redundant for Claude Code**; it remains useful for clients that load the full payload upfront. Don't chase server-side `notifications/tools/list_changed` for this client: confirmed it ignores mid-session list changes (full session restart required for any tool-set update).

### Publishing the HTTP bridge behind your own TLS proxy

If your MCP client is on a different host (or, like Claude Code, can only reach *public* HTTPS MCP servers), you'll need to put the bridge behind a TLS reverse-proxy at a public hostname. The bridge already binds `127.0.0.1:8780` and requires a bearer token (`BROWSER_MCP_HTTP_TOKEN` in `docker/.env`).

General steps:
1. **DNS:** point a hostname (e.g. `browser-mcp.example.com`) at your server.
2. **TLS reverse-proxy** (nginx / Caddy / Traefik): forward the hostname to `http://127.0.0.1:8780`. The proxy MUST: use HTTP/1.1, **disable response buffering** (SSE), keep the stream open, and pass the `Authorization` header through to the bridge. A reference nginx vhost is in [`deploy/browser-mcp.example.com.conf`](deploy/browser-mcp.example.com.conf).
3. **Register** with the client:
   ```
   claude mcp add browser-mcp -s user --transport http https://browser-mcp.example.com/mcp \
     --header "Authorization: Bearer <BROWSER_MCP_HTTP_TOKEN>"
   ```
4. Restart the client session — tool lists are loaded at startup.

Token rotation: change `BROWSER_MCP_HTTP_TOKEN` in `docker/.env` (recreate container) and the `Authorization: Bearer` header in `claude mcp add`.
