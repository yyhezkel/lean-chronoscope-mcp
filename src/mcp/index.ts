...
export function parseMcpArgs(argv: string[]): McpConfig {
  ...
  const mode: ToolMode =
    args.includes("--gateway") || env("GATEWAY") === "1"
      ? "gateway"
      : args.includes("--slim") || env("SLIM") === "1"
        ? "slim"
        : "full";
  return {
    sessionId: getArg("--session") ?? env("SESSION") ?? "default",
...
export async function runMcp(config: McpConfig): Promise<void> {
  ...
  await daemon.call("session.ensure", { sessionId: config.sessionId, source: "stdio" });
